...
void ata_wait_busy() {
    while(i386_hal_io_inb(0x1F7)&ATA_STATUS_BUSY);
}

bool ata_wait_for_data_ready() { // POLL
    for(uint8_t i = 0; i < 4; i++) {
        i386_hal_io_inb(0x3F6);
    }
    while(true) {
        uint8_t status = i386_hal_io_inb(0x1F7);
...
void ata_flush_cache() {
    i386_hal_io_outb(0x1F7, ATA_COMMAND_FLUSH);
    ata_wait_busy();
}
...
void ata_select_drive(ata_device_entry_t* ataDevice, uint32_t lba) {
    ...
    i386_hal_io_outb(ataDevice->baseIoPort + ATA_DRIVE_HEAD_REG_OFFSET, 0xE0 | (slaveBit << 4) | ((lba >> 24) & 0x0F));
}

bool ata_identify_drive(uint16_t port, bool isSlave, uint16_t* buffer) {
    uint8_t driveNum = 0xE0;
    if(isSlave) driveNum = 0xF0;
    i386_hal_io_outb(port + ATA_DRIVE_HEAD_REG_OFFSET, driveNum);
    i386_hal_io_outb(port + ATA_SECTOR_COUNT_REG_OFFSET, 0x0);
    i386_hal_io_outb(port + ATA_LBA_LOW_REG_OFFSET, 0x0);
    i386_hal_io_outb(port + ATA_LBA_MID_REG_OFFSET, 0x0);
    i386_hal_io_outb(port + ATA_LBA_HIG_REG_OFFSET, 0x0);
    i386_hal_io_outb(port + ATA_STATUS_REG_OFFSET, ATA_COMMAND_IDENTIFY);
    uint8_t status = i386_hal_io_inb(port + ATA_STATUS_REG_OFFSET);
    if(status) {
        while(i386_hal_io_inb(port + ATA_STATUS_REG_OFFSET)&ATA_STATUS_BUSY);
        while(true) {
            status = i386_hal_io_inb(port + ATA_STATUS_REG_OFFSET);
            if(status & ATA_STATUS_ERROR) {
        ...
        for(uint16_t i = 0; i < 256; i++) {
            uint16_t data = i386_hal_io_inw(port);
            buffer[i] = data;
        }
        return true;
    }
...
void ata_read_sectors_pio(ata_device_entry_t* device, uint32_t lba, uint8_t sectors, uint8_t* buffer) {
    ...
    io_wait();
	ata_wait_busy();
    i386_hal_io_outb(0x3F6, 0x02); // Disable interrupts*/
    ata_wait_busy();
    uint8_t isSlave = 0x0; 
    i386_hal_io_outb(0x1F6, 0xE0 | ((lba >> 24) & 0x0F));
    i386_hal_io_outb(0x1F1, 0x00); // This should be needed but wastes alot of cpu and according to https://wiki.osdev.org/ATA_PIO_Mode#Master.2FSlave_Drives it is ignored
    i386_hal_io_outb(0x1F2, sectors);
    i386_hal_io_outb(0x1F3, (uint8_t)lba);
    i386_hal_io_outb(0x1F4, (uint8_t)(lba >> 8));
    i386_hal_io_outb(0x1F5, (uint8_t)(lba >> 16));
    i386_hal_io_outb(0x1F7, ATA_COMMAND_READ);
    //printf("%i\n", sizeof(uint16_t));
    for(uint8_t sector = 0; sector < sectors;sector++) {
        ...
        for(uint16_t word = 0; word < 256; word++) {
            ...
            //printf("%i,", index);
            uint16_t data = i386_hal_io_inw(0x1F0);
...
void ata_write_sectors_pio(ata_device_entry_t* device, uint32_t lba, uint8_t sectors, uint16_t* buffer) {
    ata_wait_busy();
    uint8_t isSlave = 0x0;
    i386_hal_io_outb(0x1F6, 0xE0 | (isSlave << 4) | ((lba >> 24) & 0x0F));
    //i386_hal_io_outb(0x1F1, 0x00); // This should be needed but wastes alot of cpu and according to https://wiki.osdev.org/ATA_PIO_Mode#Master.2FSlave_Drives it is ignored
    i386_hal_io_outb(0x1F2, sectors);
    i386_hal_io_outb(0x1F3, lba & 0xFF);
    i386_hal_io_outb(0x1F4, (lba >> 8) & 0xFF);
    i386_hal_io_outb(0x1F5, (lba >> 16) & 0xFF);
    i386_hal_io_outb(0x1F7, ATA_COMMAND_WRITE);
     ...
     for(uint8_t sector = 0; sector < sectors;sector++) {
        ...
        if(!ata_wait_for_data_ready()) return;
        for(uint16_t word = 0; word < 256; word++) {
            i386_hal_io_outw(0x1F0, buffer[word+sectorOffset]);
            //ata_flush_cache();
        }
    }
}
...
block_device_t* ata_create_device_object(uint16_t port, bool isSlave, uint16_t* buffer) {
    if(ata_identify_drive(port, isSlave, buffer)) {
        ...
        ata_device_entry_t* ataDevice = kmalloc(sizeof(ata_device_entry_t));
        ...
        printf("Found ATA Device Mode %s Max Sectors %#08X%08X\n", mode, maxSectorsHigh, maxSectorsLow);
...
void ata_init_devices() {
    // Used as byte buffer 
    uint8_t* identifyBuffer = kmalloc(512);
...
void ata_block_write(block_device_t* device, uint32_t blockOffset, uint32_t blockCount, void* buffer) {
    ata_device_entry_t* ataDevice = (ata_device_entry_t*)device->hwDevicePtr;
    ata_write_sectors_pio(ataDevice, blockOffset, blockCount, (uint8_t*)buffer);
}