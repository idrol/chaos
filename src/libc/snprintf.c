...
void addCharToBuffern(char* buffer, size_t bufferSize, const char c, int* a) {
    if(*a < bufferSize)buffer[*a] = c;
    *a += 1;
}

void addStringToBuffern(char* buffer, size_t bufferSize, const char* c, int* a) {
    for(int i = 0; c[i]; ++i) {
        addCharToBuffern(buffer, bufferSize, c[i], a);
    }
}

int vsnprintf (char* buffer, size_t bufferSize, const char* format, va_list list) {
    int chars              = 0;
    ...
    for (int i = 0; format[i]; ++i) {
        ...
        if (format[i] == '%') {
            ...
            if (specifier == 'o') {
                ...
                specifier = 'u';
                if (altForm) {
                    addStringToBuffern(buffer, bufferSize, "0", &chars);
                }
            }
            if (specifier == 'p') {
                base = 16;
            ...
            switch (specifier) {
                ...
                case 'x':
                    ...
                    if (altForm) {
                        addStringToBuffern(buffer, bufferSize, "0x", &chars);
                    }
                case 'u':
                {
                    switch (length) {
                        case 0:
                        {
                            ...
                            __uint_str(integer, intStrBuffer, base, spaceNoSign, lengthSpec, leftJustify, zeroPad);
                            addStringToBuffern(buffer, bufferSize, intStrBuffer, &chars);
                            break;
                        }
                        case 'H':
                        {
                ...
                {
                    if (length == 'l') {
                        //TODO displayCharacter(va_arg(list, wint_t), &chars);
                    } else {
                        addCharToBuffern(buffer, bufferSize, va_arg(list, int), &chars);
                    }
                ...
                case 's':
                {
                    addStringToBuffern(buffer, bufferSize, va_arg(list, char*), &chars);
                    break;
                }
                ...
                case 'g':
                case 'G':
                    // Uncomment when floating point calculations are available
                    /*
                    {
            ...
            if (specifier == 'e') {
                addStringToBuffern(buffer, bufferSize, "e+", &chars);
            } else if (specifier == 'E') {
                addStringToBuffern(buffer, bufferSize, "E+", &chars);
            }
            ...
            addCharToBuffern(buffer, bufferSize, format[i], &chars);
        }
    }
    addCharToBuffern(buffer, bufferSize, '\0', &chars);
